...
namespace hotel3g.Controllers
{
    public class MemberCardController : Controller
    {
        ...
        public ActionResult CardList()
        {
            ...
            //maxCardLevel = MemberCardBuyRecord.GetMaxCardLevel(hotelweixinId, userweixinId);
        ...
        public ActionResult CardBuy()
        {
            ...
            if (db.Rows.Count == 0)
            {
                return RedirectToAction("CardList", "MemberCard", new { id = RouteData.Values["id"], key = HotelCloud.Common.HCRequest.GetString("key") });
            }
            ...
            if (xx)
            {
                //MemberCardBuyRecord.DoneOrderSuccess("K17062214113777891");
            }
        ...
        public ActionResult MemberRestPassword(string id)
        {
            ...
            ViewData["MemberCenter"] = string.Format("/User/Index/{0}?key={1}", id, key);
            string restphone = hotel3g.Models.Cookies.GetCookies("restphone", "");
            if (string.IsNullOrEmpty(restphone))
            {
                return RedirectToAction("PwdRest", "MemberCard", new { id = id, key = key });
            }
            return View();
        }
        // GET: /MemberCard/
        ...
        public ActionResult MemberRegister(string id)
        {
            ...
            else
            {
                ...
                {
                    return RedirectToAction("CardList", "MemberCard", new { id = id, key = key });
                }
            }
...
SELECT 0 FROM dbo.ShareCouponContent WITH(NOLOCK) WHERE tel=a.mobile
            ...
            {
                ViewData["MemberCenter"] = string.Format("/User/MyCouPons/{0}?key={1}", id, key);
            }
            else
            {

                ViewData["MemberCenter"] = string.Format("/User/Index/{0}?key={1}{2}", id, key, query);
            }
            ViewData["weixinID"] = weixinID;
            ViewData["hId"] = id;
            ViewData["userWeiXinID"] = userWeiXinID;
        ...
        public ActionResult MemberCenterLogin(string id)
        {
            ...
            string MemberCenterUrl = string.Format("/User/Index/{0}?key={1}", id, key);
            string MemberInfoCenter = string.Format("/MemberCard/MemberCenter/{0}?key={1}", id, key);
            string PwdRest = string.Format("/MemberCard/PwdRest/{0}?key={1}", id, key);
            string HomeMain = string.Format("/Home/Main/{0}?key={1}", id, key);
        ...
        public ActionResult MemberCenter(string id)
        {
            ...
            if (keys.Length >= 2)
            {
                ...
                ViewData["Info"] = Info;
                string HomeMainUrl = string.Format("/Home/Main/{0}?key={1}", id, key);
                ViewData["HomeMainUrl"] = HomeMainUrl;

                string MyOrdersUrl = string.Format("/User/MyOrders/{0}?key={1}", id, key);
                ...
                if (!string.IsNullOrEmpty(cardno))
                {
                    ...
                    string jifendeatilurl = string.Format("/MemberCard/JiFendetail/{0}?key={1}", id, key);
                    ViewData["jifendeatilurl"] = jifendeatilurl;
                    string EditMemberCardUrl = string.Format("/MemberCard/EditMemberCard/{0}?key={1}", id, key);
        ...
        public ActionResult RandomLuckyDraw(string id)
        {
            ...
            ViewData["userinfo"] = string.Format("/MemberCard/UserLuckyDrawInfo/{0}?key={1}&drawid={2}", id, key, drawid);
        ...
        public ActionResult LuckyDrawSignUp(string id)
        {
            ...
            ViewData["RandomLuckyDrawUrlNew"] = string.Format("/MemberCard/RandomLuckyDraw/{0}?key={1}&drawid={2}", id, key, drawid);
        ...
        public ActionResult Authentication()
        {
            ...
            if (string.IsNullOrEmpty(code))
            {
                ...
                ///静默授权返回页
                string redirect_uri = HttpUtility.UrlEncode(string.Format("http://hotel.weikeniu.com/MemberCard/Authentication?weixinid={0}", WeiXinId));
                //获取授权
        ...
        public ActionResult Authentication1()
        {
            ...
            if (string.IsNullOrEmpty(code))
            {
                ...
                ///静默授权返回页
                string redirect_uri = HttpUtility.UrlEncode(string.Format("http://hotel.weikeniu.com/MemberCard/Authentication1?weixinid={0}{1}", WeiXinId, queryid));
                //获取授权
                string url = string.Format("https://open.weixin.qq.com/connect/oauth2/authorize?appid={0}&redirect_uri={1}&response_type=code&scope=snsapi_base&state={2}#wechat_redirect", Config.appid, redirect_uri, target);
                return Redirect(url);
            }
            else
            {
                ...
                else if (state.ToLower().IndexOf("productdetail") > -1)
                {
                    query2 = id > 0 ? "&ProductId=" + id.ToString() : "";
                }
