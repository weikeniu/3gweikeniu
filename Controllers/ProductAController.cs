...
namespace hotel3g.Controllers
{
    public class ProductAController : Controller
    {
        ...
        public ActionResult ProductDetail()
        {
            ...
            {
                //return RedirectToAction("ProductErrMsg", "ProductA", new { id = RouteData.Values["id"], errmsg = "没找到对应的产品", key = HotelCloud.Common.HCRequest.GetString("key") });
                return RedirectToAction("productList", "ProductA", new { id = RouteData.Values["id"], key = HotelCloud.Common.HCRequest.GetString("key") });
            }

            if (products.EndTime < DateTime.Now)
            {
                //return RedirectToAction("ProductErrMsg", "ProductA", new { id = RouteData.Values["id"], errmsg = "产品已过期", key = HotelCloud.Common.HCRequest.GetString("key")  });
            }

            if (products.Status != 0)
            {
                //return RedirectToAction("ProductErrMsg", "ProductA", new { id = RouteData.Values["id"], errmsg = "产品已下架", key = HotelCloud.Common.HCRequest.GetString("key") });
                return RedirectToAction("productList", "ProductA", new { id = RouteData.Values["id"], key = HotelCloud.Common.HCRequest.GetString("key") });
            }
            ...
            if (minPrice == 0)
            {
                //return RedirectToAction("ProductErrMsg", "ProductA", new { id = RouteData.Values["id"], errmsg = "产品已售完", key = HotelCloud.Common.HCRequest.GetString("key") });
                return RedirectToAction("productList", "ProductA", new { id = RouteData.Values["id"], key = HotelCloud.Common.HCRequest.GetString("key") });
        ...
        public ActionResult ProductConfirmOrder()
        {
            ...
            if (products == null || products.Id == 0)
            {
                //return RedirectToAction("ProductErrMsg", "ProductA", new { id = RouteData.Values["id"], errmsg = "没有找到对应的产品。", key = HotelCloud.Common.HCRequest.GetString("key") });
                return RedirectToAction("productList", "ProductA", new { id = RouteData.Values["id"], key = HotelCloud.Common.HCRequest.GetString("key") });
            }
            ...
            if (products.List_SaleProducts_TC == null || products.List_SaleProducts_TC.Count == 0)
            {
                //return RedirectToAction("ProductErrMsg", "ProductA", new { id = RouteData.Values["id"], errmsg = "没有找到对应的套餐。", key = HotelCloud.Common.HCRequest.GetString("key") });
                return RedirectToAction("productList", "ProductA", new { id = RouteData.Values["id"], key = HotelCloud.Common.HCRequest.GetString("key") });
            }
