    ...
    public class UserController : Controller
    {
        ...
        public JsonResult GetOrderListData()
        {
            ...
            if (OrderList.Count > 0)
            {
                ...
                string json = Newtonsoft.Json.JsonConvert.SerializeObject(PackOrderList);
        ...
        public ActionResult OrderInfo(string id)
        {
            ...
            ViewData["comment"] = 0;
            ViewData["commentopen"] = 0;

            if (order.State == 9)
            {
                string sql = "select count(*) from Comment with (nolock) where hotelid=@hotelid and OrderId=@OrderId ";
                string commentCount = SQLHelper.Get_Value(sql, SQLHelper.GetCon(), new Dictionary<string, DBParam>()
                {
                    {"hotelid",new DBParam(){ParamValue=order.HotelID.ToString()}},
                    {"OrderId",new DBParam(){ParamValue=order.ID.ToString()}}
                });
                ViewData["comment"] = Convert.ToInt32(commentCount);
        ...
        public ActionResult Sports(string id)
        {
            ...
            if (dt != null)
            {
                ...
                {
                    //
                    return Redirect(string.Format("/Hotel/PrizeSports/{0}?key={1}", id, userWeiXinID));
                    //return Redirect("/User/Error/" + id + "?weixinID=" + weixinid);
                }
            }
            ViewData["weixinid"] = weixinid;
        ...
        public ActionResult UserCollection()
        {
            ...
            var dic = new Dictionary<string, HotelCloud.SqlServer.DBParam> { 
             ...
             {"HotelId",new HotelCloud.SqlServer.DBParam{ParamValue=hid.ToString()}},
             {"weixinId",new HotelCloud.SqlServer.DBParam{ParamValue=hotelweixinId}}
            };

            DataTable db_c = HotelCloud.SqlServer.SQLHelper.Get_DataTable("select name,url  from CollectionHotel with(nolock)  where userweixinId=@userweixinId    and  Status=1 ", HotelCloud.SqlServer.SQLHelper.GetCon(), dic);
