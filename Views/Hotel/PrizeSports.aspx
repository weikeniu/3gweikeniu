...
<body>   
    <div class="all" id="contain_all">
            ...
            if (mlist != null)
            {
                ...
                {%>
                   <div class="cshdlb detail_dp" data-id="<%=item.Id %>">
		            <div class="cshdlbbt">
			            <strong><%=item.Title %><%=item.Exclusive==1?" (会员专享)":"" %></strong>
		            </div>
		            <div class="cshdlbtp">
                        <%
                    if (item.Type == "dazhuanpan")
                    {%>
                        <a href="javascript:void(0)"><img src="../../img/dazhuanpan.png" style="width: 100%; height:150px"/></a>
                    <%}
                    else
                    {%>
                         <a href="javascript:void(0)"><img src="../../img/guagua.jpg" style="width: 100%; height:150px"/></a>
                    <%}
                             %>
		            </div>
	            </div> 
        ...
        <div class="cshdlb" data-id="">
		            <div class="cshdlbbt">
			            <strong><%=HotelLuckyDraws[i].title %></strong>
		            </div>
		            <div class="cshdlbtp">
                       
                        <a href="/MemberCard/LuckyDrawSignUp/<%=hotelid %>?&key=<%=ViewData["weixinID"] %>@<%=ViewData["userWeiXinID"] %>&drawid=<%=HotelLuckyDraws[i].id %>"><img src="/img/zhuanpan.jpg" style="width: 100%; height:150px"/></a> 
		            </div>
	            </div> 
...
$(function () {
    $('#contain_all').on('click', 'div[class*="detail_dp"]', function () {
        ...
        location.href = '/User/Sports/<%=ViewData["hId"] %>?sid=' + id + '&key=<%=ViewData["weixinID"]%>@<%=ViewData["userWeiXinID"] %>';
    });
