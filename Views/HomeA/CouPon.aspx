                    ...
                    <% foreach (System.Data.DataRow row in CouponList.Rows)
                      ...
                      <li class="ca-red-bag">
                                    <div class="ca-redbag-amount">
                                          ...
                                          <% if (!string.IsNullOrEmpty(amountlimit) && !amountlimit.Equals("0"))
                                             { %>
                                          满<%=amountlimit %>可用
                                          <%} %>
                                          </div>
                            ...
                            %><div class="ca-gz-fding ca-flex <%= scopeArry.Count>1 && scopeArry.Count<5?"ca-mart-line-small":"ca-mart-line-big" %> ">
                                             <% 
                                                 ...
                                                 string link = string.Format("/HotelA/Index/{0}?key={1}@{2}", ViewData["hId"], ViewData["weixinID"], ViewData["userWeiXinID"]);
                                                 ...
                                                 if (scopeArry.Count > 0)
                                                 {
                                                     ...
                                                     else
                                                     {
                                                         ...
                                                         {
                                                             case "1":
                                                                 link = string.Format("/DishOrderA/DishOrderIndex/{0}?key={1}@{2}", ViewData["hId"], ViewData["weixinID"], ViewData["userWeiXinID"]);
                                                                 break;
                                                             case "2":
                                                                 link = string.Format("/SupermarketA/Index/{0}?key={1}@{2}", ViewData["hId"], ViewData["weixinID"], ViewData["userWeiXinID"]);
                                                                 break;
                                                             case "3":
                                                                 link = string.Format("/ProductA/ProductList/{0}?key={1}@{2}", ViewData["hId"], ViewData["weixinID"], ViewData["userWeiXinID"]);
                                                                 break;
                                                             case "4":
                                                                 link = string.Format("/DishOrderA/DishOrderIndex/{0}?key={1}@{2}", ViewData["hId"], ViewData["weixinID"], ViewData["userWeiXinID"]);
                                                                 break;
    ...
    <%}
       ...
       { %>
             <div class="ca-no-redbag">
                  <span style="background:#ccc"><em></em></span><p>暂时没有可领取红包哦</p>
             </div>
       <%} %>
