...
%>
    ...
    <div class="zone__goods-details">
        ...
        <div class="g-title bg--fff">
            <div class="inner yu-grid">
                ...
                <label>
                    已售
                    <%if (string.IsNullOrWhiteSpace(ViewData["soldCount"].ToString()))
                      { %>
                      0
                    <%}
                      else
		...
		<div class="g-subinfo bg--fff mt20">
			<label><span><i class="ico i1 i1-1"></i>快递</span><span><i class="ico i1 i1-1"></i>到店自提</span></label>
		</div>
		
		<div class="g-subinfo bg--fff mt20">
			<label><span><i class="ico i2 i2-2"></i>此商品由<%=ViewData["hotelName"]%>提供</label>
		</div>
        <!--订购须知-->
        ...
        <div class="g-footinfo fixed">
            ...
            <% if (!ViewData["userweixinid"].ToString().Contains(wkn_shareopenid)) { %>
                ...
                <span><a class="ico i1 i1-1" href="/homeA/main/<%=ViewData["hotelId"] %>?key=<%=ViewData["weixinid"] %>@<%=ViewData["userweixinid"] %>">
                </a></span><span><a class="ico i2 i2-2" href="/userA/myorders/<%=ViewData["hotelId"] %>?key=<%=ViewData["weixinid"] %>@<%=ViewData["userweixinid"] %>">
                </a></span>
            ...
            <%} %>
                ...
                <span class="yu-overflow">
                    <button class="g-buy">售罄</button></span>
                <%} %>
            </div>
        </div>
    </div>
    ...
    $(function(){
	    ...
	    function GotoMap() { 
            location.href = "/hotelA/map/<%=ViewData["hotelId"] %>?key=<%=ViewData["weixinid"] %>@<%=ViewData["userweixinid"] %>";
        }

        function PayByScore(){
                    sessionStorage.SupperMarketIsBack = 0;
            location.href = "/SupermarketA/OrderDetailsAlone/<%=ViewData["hotelId"] %>?key=<%=ViewData["weixinid"] %>@<%=ViewData["userweixinid"] %>&commodityid=<%=ViewData["CommodityID"] %>&PayMode=score";
        }
        
        function PayByMoney(){
                    sessionStorage.SupperMarketIsBack = 0;
            location.href = "/SupermarketA/OrderDetailsAlone/<%=ViewData["hotelId"] %>?key=<%=ViewData["weixinid"] %>@<%=ViewData["userweixinid"] %>&commodityid=<%=ViewData["CommodityID"] %>&PayMode=money";
        }
    </script>
    <!-- 左右滑屏.End -->
</body>
